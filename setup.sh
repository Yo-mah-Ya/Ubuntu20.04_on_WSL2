...
readonly AWS_CLI_V2_for_X86_64_latest_version_would_be_installed=true 2> /dev/null
...
readonly SSL_SCAN_would_be_installed=false 2> /dev/null
...
sudo apt install -y libssl-dev libcurl4-openssl-dev liblog4cxx-dev \
                ...
                jq tree nfs-common zip unzip zsh
...
#######################################################
if "${AWS_CLI_V2_for_X86_64_latest_version_would_be_installed}" ; then
...
# ssl-scan
#######################################################
if "${SSL_SCAN_would_be_installed}" ; then
    cd ~
